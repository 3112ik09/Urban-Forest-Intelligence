...
export default function ProgressPanel({
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      ...
      {/* Header */}
      <div>
        <h2 className="text-base font-semibold text-gray-900 leading-tight">{district}</h2>
        ...
        {STEPS.map((step, i) => {
          ...
          return (
            ...
            >
              {/* Step icon */}
              {done ? (
                <div className="w-5 h-5 shrink-0 rounded-full bg-green-100 border border-green-300 flex items-center justify-center">
                  <span className="text-[10px] text-green-600 font-bold leading-none">✓</span>
                </div>
              ) : active ? (
                <div className="w-5 h-5 shrink-0 rounded-full border-2 border-green-200 border-t-green-500 animate-spin" />
              ) : (
                <div className="w-5 h-5 shrink-0 rounded-full bg-gray-100 border border-gray-200 flex items-center justify-center">
                  <span className="text-[10px] text-gray-400 leading-none">{stepNum}</span>
                </div>
              )}
              ...
              <div className="flex-1 min-w-0">
                ...
                {active && detail && (
                  <p className="text-[10px] text-blue-600 mt-0.5 leading-snug">{detail}</p>
                )}
              </div>
